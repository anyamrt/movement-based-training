import { useModal } from '../hooks/useModal';
import ContactFormModal from './ContactFormModal';

const Contact = () => {
  const { isOpen: isContactOpen, openModal: openContactModal, closeModal: closeContactModal } = useModal();

  return (
    <section id="contact" className="py-20 bg-gradient-to-b from-brand-light to-white">
      <div className="container mx-auto px-4 max-w-6xl">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-bold text-brand-dark mb-4">
            How to Get Started
          </h2>
          <div className="w-24 h-1 bg-brand-primary mx-auto mb-6"></div>
          <p className="text-xl text-gray-600 max-w-2xl mx-auto">
            Ready to level up your training? Get in touch to book your session or learn more.
          </p>
        </div>

        <div className="grid md:grid-cols-2 gap-12 mb-12">
          {/* Contact Info */}
          <div className="space-y-8">
            <div>
              <h3 className="text-2xl font-bold text-brand-dark mb-6">Get In Touch</h3>
              <div className="space-y-4">
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 bg-brand-primary text-white rounded-lg flex items-center justify-center flex-shrink-0">
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                    </svg>
                  </div>
                  <div>
                    <h4 className="font-semibold text-brand-dark mb-1">Location</h4>
                    <p className="text-gray-600">43 godwin street</p>
                    <p className="text-gray-600">Bulimba 4171</p>
                  </div>
                </div>

                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 bg-brand-primary text-white rounded-lg flex items-center justify-center flex-shrink-0">
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                    </svg>
                  </div>
                  <div>
                    <h4 className="font-semibold text-brand-dark mb-1">Email</h4>
                    <a href="mailto:movementbasedtraining@gmail.com" className="text-brand-primary hover:text-brand-accent transition-colors">
                      movementbasedtraining@gmail.com
                    </a>
                  </div>
                </div>

                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 bg-brand-primary text-white rounded-lg flex items-center justify-center flex-shrink-0">
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                  </div>
                  <div>
                    <h4 className="font-semibold text-brand-dark mb-1">Training Hours</h4>
                    <p className="text-gray-600">Flexible scheduling available</p>
                    <p className="text-gray-600">Early morning & evening sessions</p>
                  </div>
                </div>
              </div>
            </div>

            {/* Social Links */}
            <div>
              <h4 className="font-semibold text-brand-dark mb-4">Connect With Me</h4>
              <div className="flex gap-4">
                <a
                  href="https://www.instagram.com/james_ashford_pt"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-12 h-12 bg-brand-primary hover:bg-brand-accent text-white rounded-lg flex items-center justify-center transition-all transform hover:scale-110"
                  aria-label="Instagram"
                >
                  <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                  </svg>
                </a>
              </div>
            </div>
          </div>

          {/* CTA Card */}
          <div className="bg-gradient-to-br from-brand-primary to-brand-accent rounded-3xl shadow-2xl p-8 md:p-12 text-white">
            <h3 className="text-3xl font-bold mb-4">Ready to Start Your Journey?</h3>
            <p className="text-lg mb-8 text-white/90">
              Book your first session or reach out with any questions. Let's work together
              to unlock your potential and level up your fitness.
            </p>

            <div className="space-y-4">
              <button
                onClick={openContactModal}
                className="block w-full bg-white hover:bg-gray-100 text-brand-primary py-4 px-6 rounded-full font-semibold text-center transition-all transform hover:scale-105 shadow-lg"
              >
                Send us a Message
              </button>
              <p className="text-center text-sm text-white/75">
                Get in touch with any questions or inquiries
              </p>
            </div>
          </div>
        </div>

        {/* Map Section */}
        <div className="mt-16">
          <h3 className="text-3xl font-bold text-brand-dark mb-6 text-center">
            Find us
          </h3>
          <div className="bg-white rounded-3xl shadow-xl overflow-hidden">
            <iframe
              src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3540.6789!2d153.0619!3d-27.4553!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x6b915a1c3df7a9e5%3A0xf02a35bd720a340!2s43%20godwin%20street%2C%20Bulimba%20QLD%204171%2C%20Australia!5e0!3m2!1sen!2sau"
              width="100%"
              height="400"
              style={{ border: 0 }}
              allowFullScreen={true}
              loading="lazy"
              referrerPolicy="no-referrer-when-downgrade"
              title="Location Map - 43 godwin street, Bulimba 4171"
            ></iframe>
          </div>
          <p className="text-center text-gray-600 mt-4">
            <strong>Think 24hr Fitness, 43 Godwin Street, Bulimba</strong>
          </p>
        </div>
      </div>

      {/* Contact Modal */}
      <ContactFormModal isOpen={isContactOpen} onClose={closeContactModal} />
    </section>
  );
};

export default Contact;
